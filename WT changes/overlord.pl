...
sub check_all{
	...
	unless($rh->{"openjdk8"}){
		append_log("Trying to start greasyspoon");
		
                `/usr/local/bin/bash /root/greasyspoon-release-1.0.10/greasyspoon start`;
				#my $env_prefix="";
		...
		append_log("greasyspoon started ");
	}
...
sub stop_daemons{
	...
	append_log("Trying to stop greasyspoon");
	my $cnt = 0;
	#while($cnt++ < 5 and pkill("icapsvr")){
	#	sleep 1;
	#}
	`/usr/local/bin/bash /root/greasyspoon-release-1.0.10/greasyspoon stop`;
